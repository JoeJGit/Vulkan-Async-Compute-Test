...
void ImGui::RangeBar(float fraction0, float fraction1, const ImVec2& size_arg, ImU32 color)
{
    ImGuiWindow* window = GetCurrentWindow();
    if (window->SkipItems)
        return;
    ...
    // Render
    fraction0 = ImSaturate(fraction0);
    fraction1 = ImSaturate(fraction1);
    RenderFrame(bb.Min, bb.Max, GetColorU32(ImGuiCol_FrameBg, 0.5f), true, style.FrameRounding);
    bb.Reduce(ImVec2(window->BorderSize, window->BorderSize));
    const ImVec2 fill_br0 = ImVec2(ImLerp(bb.Min.x, bb.Max.x, fraction0), bb.Max.y);
    const ImVec2 fill_br1 = ImVec2(ImLerp(bb.Min.x, bb.Max.x, fraction1), bb.Min.y);
    RenderFrame(fill_br1, fill_br0, color, false, style.FrameRounding);
}
...
//bool ImGui::InputIntS(const char * label, int * v, int step, int step_fast, ImGuiInputTextFlags extra_flags)
//{
//	int temp = *v;
//	bool r = InputInt (label, &temp, step, step_fast, extra_flags | ImGuiInputTextFlags_EnterReturnsTrue);
//	if (r) *v = temp;
//	return r;
//}
